...
- カラム単位のチャート/サマリーコンポーネント（`src/charts/`）は、クライアント側で統計量を再計算するのではなく、Rust側のsummarise関数群が生成する `ColumnSummary` のバリアントをそのまま消費する（`get_table_metadata`の`summary`フィールド、こちらは元々テーブル全体に対する集計でありページングの影響を受けない）。
