...
- `src-tauri/db`（workspaceメンバー、クレート名 `db`）: DuckDB/Arrow関連のロジックをすべて持つ、フレームワーク非依存の層。`DbState` がDuckDBのコネクションをラップし、データに関するあらゆる処理を担う — `register_data`（ファイルをテーブルとして読み込む）、`execute`/`execute_with_save`、スキーマ取得、カラムごとの統計サマリー（`numeric_summarise`、`temporal_summarise`、`string_summarise`、`boolean_summarise`、`other_summarise`、`binning`、`value_counts`、`value_counts_limited`）など。カラムのdtypeマッピング/パースは `duckdb_data_type.rs` にある。
  - 注意: 列のサマリー(`ColumnSummary`)には生データは含まれない。`numeric_summarise`/`temporal_summarise`は統計量・SQL集計済みのビンのみを返し、`string_summarise`は`value_counts_limited`(上位50件+Other1件に頭打ち)を使う。ヒストグラムのビン数変更・範囲フィルタ等インタラクティブな再計算は、都度専用コマンド(`get_numeric_bins`)経由でバックエンドへ再クエリする設計にしている(「サーバー側で計算し、フロントには表示分だけ渡す」方針、詳細は`docs/design/performance.md`の「ヒストグラム/棒グラフのインタラクティブ機能をサーバー側計算に移行」参照)。
  - 注意: `DbState::summarise_all`（`get_table_metadata`/`execute_query`が全カラムのsummariseをまとめて呼ぶ入り口）は、列ごとの集計クエリを`rayon`で並列実行する。DuckDBの`Connection`は`Send`だが`Sync`ではない（内部に`RefCell`を持つ）ため、`&DbState`を複数スレッドから同時参照する形の並列化はできない。そのため、並列化に入る前に`Connection::try_clone()`（同一データベースを共有する新しいコネクション、in-memoryの場合も含めて共有される）を**逐次**用意し、各コネクションの所有権をタスクへそのまま渡す設計にしている。コネクション数は列数ではなく`rayon::current_num_threads()`で頭打ちにしており、列をワーカー数でチャンク分割して各チャンクを1コネクションで逐次処理する（列幅の広いテーブルで無駄なコネクションを大量に張らないため）。詳細・実測は`docs/design/performance.md`の「summariseの列並列化 実施後の再計測結果」を参照。
- `src-tauri/src`（クレート名 `data_viewer_lib`、Cargo workspaceのルートパッケージ）: Tauriとのつなぎ込み層。`src/modules/handler.rs` がフロントエンドから `invoke(...)` 経由で呼ばれる `#[tauri::command]` 関数群（`register_data`、`execute_query`、`get_table_metadata`、`fetch_table_page`、`fetch_row_range`、`count_table_rows`、`get_numeric_bins`、`export_table_csv`、`get_table_names`、`get_status`、`get_duckdb_symbols`、`sql_lint`、`sql_fix`、`save_text_file`、`save_database`、`open_database`、`drop_table`、`rename_table`、`new_in_memory_database`、`get_settings`、`set_settings`、`switch_http_port`、`log_frontend_perf`）を公開しており、`DbState` を `AppData` でラップしてTauriのmanaged state内で `Mutex` 越しに保持している。
  - 注意: `app_log`/`app_log_perf`はログビューア用に`AppData.logs`へ書き込むため内部で`Mutex<AppData>`を再度lockする。したがって、`state.lock()`で得たガードを保持したままのスコープ内で`app_log`系を呼ぶと自己デッドロックする(`std::sync::Mutex`は再入不可)。`build_table_metadata_json`のように処理時間計測をロック内で行いたい場合は、`PerfLog`(`Vec<(LogLevel, String, Duration)>`)に一旦バッファし、ロックを解放してから`flush_perf_log`でまとめて出力する（詳細は`docs/design/performance.md`の実測結果を参照、実際にこのバグを一度作り込んで発見した）。
...
`src/handler.ts` がReactとRustの唯一の境界であり、すべての `invoke()` 呼び出しはここに集約されている。メインテーブル表示はサーバー側ページング化されており、DuckDB側でOFFSET/LIMIT/ORDER BY/WHEREを処理し、フロントには表示中の少量データだけを渡す設計になっている（`docs/design/performance.md`の「サーバー側ページング化」参照、大規模データでクライアント側に全行を持つ設計が支配的コストになっていたことが実測で判明し、POC検証を経て移行した）。

- `get_table_metadata`/`execute_query`コマンドは、テーブル（または`execute_query`が`_last`という一時テーブルに保存したクエリ結果）の`name`/`schema`/`summary`/`totalRows`（フィルタなしの全行数）のみを返す。**メインテーブルの全行データ(df)はここには含まれない**。戻り値は通常の`Result<T, InvokeError>`ではなく`tauri::ipc::Response`を使っている。`build_table_metadata_json`（`src-tauri/src/modules/handler.rs`）がこれらを文字列連結で1つのJSONオブジェクトテキストとして手組みし、`Response::new(...)`でそのままレスポンスbodyとして返す。フロント側はTauriのIPCブリッジ（`content-type: application/json` → `response.json()`）が自動でパースするため、`src/handler.ts`側で`JSON.parse`を手動で呼ぶ必要はない。この`Response`経由の設計は、素朴に`Result<T, InvokeError>`で返すとTauri側のコマンド戻り値シリアライズでJSON文字列がもう一段エスケープされ直すことが実測で判明し（`docs/design/performance.md`参照）、その対策として導入した。
- `App.tsx`がトップレベルの状態（現在のテーブルのメタデータ、テーブル名一覧、クエリ、ステータス）を保持し、バックエンドの`update-data`イベント（上記3つのエントリーポイントいずれかから発火される）を監視して新しいファイルが読み込まれたことを検知し、テーブル名と現在のテーブルのメタデータを再取得する。
- メインテーブルのデータ本体は`src/Table.tsx`（`DataTable`）が`tableName`propから自前で取得する。共有フック`src/usePagedRows.ts`が、`fetch_table_page`コマンド（指定したoffset/limit/sort/whereで絞り込んだ行のJSON配列テキストをそのまま`Response`で返す）を使ってページ（200行）単位でフェッチし、TanStack Tableに渡す`data`配列の長さを常に`totalRows`（サーバー側フィルタ後の総件数）に保ちながら、未ロードの行は共有の空オブジェクト`LOADING_ROW`で埋めておく設計になっている。これにより`row.index`が常に「サーバー側ソート/フィルタ済み結果内での絶対位置」と一致し、行番号表示・仮想化（`@tanstack/react-virtual`）のcountがそのまま使える。列ヘッダーのソートクリック・検索ボックス（グローバル検索）・高度フィルタ（`src/advancedFilter.ts`の`conditionsToSql`/`globalSearchToSql`）はSQLのORDER BY/WHERE句に変換されてサーバー側で評価され、クライアント側でのJS配列フィルタ/ソートは行わない。`Table.tsx`のGrid・`GlimpseView.tsx`（転置ビュー）・`RecordView.tsx`（1レコード表示）はいずれもこの同じ`usePagedRows`の結果（`rows`/`table`）を共有し、それぞれ自身の可視範囲（縦/横スクロール、または表示中の1行）に応じて`requestRange`を呼びプリフェッチする。
- セル範囲選択のコピー（`src/useCellRangeSelection.ts`）は、選択範囲が未ロードの行を含みうるため、`fetch_row_range`コマンド（選択範囲・列を1クエリで取得）を使って常にサーバーへ直接問い合わせる（ページキャッシュは経由しない）。選択行数が閾値（5万行）を超える場合は確認ダイアログ（`src/components/LargeCopyConfirmDialog.tsx`）を挟む。
- CSV全件エクスポート（`src/components/ExportActions.tsx`）は、`export_table_csv`コマンドが現在のソート/フィルタ条件に一致する全行をDuckDBの`COPY ... TO ... (FORMAT CSV, HEADER)`で直接ファイルへ書き出す。JS側にデータを一切渡さないため、ロード済みかどうかに関わらず全件を高速にエクスポートできる。
- カラム単位のチャート/サマリーコンポーネント（`src/charts/`）は、クライアント側で統計量を再計算するのではなく、Rust側のsummarise関数群が生成する `ColumnSummary` のバリアントをそのまま消費する（`get_table_metadata`の`summary`フィールド、こちらは元々テーブル全体に対する集計でありページングの影響を受けない）。ヒストグラムの詳細モーダル（`HistogramChartInteractive`）のように、ユーザー操作（ビン数変更・範囲フィルタ）に応じて集計をやり直す必要がある場合は、生データをクライアントに持たせず、共有フック（`src/useNumericBins.ts`、`usePagedRows.ts`と同じ120msデバウンス+generationカウンタ規約）経由で専用コマンド（`get_numeric_bins`）へ再クエリする。今後追加する対話的チャートもこの形に倣う想定。
