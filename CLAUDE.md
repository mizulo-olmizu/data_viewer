...
- `src-tauri/src`（クレート名 `data_viewer_lib`、Cargo workspaceのルートパッケージ）: Tauriとのつなぎ込み層。`src/modules/handler.rs` がフロントエンドから `invoke(...)` 経由で呼ばれる `#[tauri::command]` 関数群（`register_data`、`execute_query`、`extract_table`、`get_table_names`、`get_status`、`get_duckdb_symbols`、`sql_lint`、`sql_fix`、`save_text_file`、`save_database`、`open_database`、`drop_table`、`rename_table`、`new_in_memory_database`、`get_settings`、`set_settings`、`switch_http_port`、`log_frontend_perf`）を公開しており、`DbState` を `AppData` でラップしてTauriのmanaged state内で `Mutex` 越しに保持している。
  - 注意: `app_log`/`app_log_perf`はログビューア用に`AppData.logs`へ書き込むため内部で`Mutex<AppData>`を再度lockする。したがって、`state.lock()`で得たガードを保持したままのスコープ内で`app_log`系を呼ぶと自己デッドロックする(`std::sync::Mutex`は再入不可)。`extract_data`のように処理時間計測をロック内で行いたい場合は、`PerfLog`(`Vec<(LogLevel, String, Duration)>`)に一旦バッファし、ロックを解放してから`flush_perf_log`でまとめて出力する（詳細は`docs/design/performance.md`の実測結果を参照、実際にこのバグを一度作り込んで発見した）。
