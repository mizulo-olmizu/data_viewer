...
- **③ summariseの列並列化(優先度を引き上げ、2026-07-28)**: 上記の理由により、ページング化前は「全体の1割未満」だったsummariseのコストが、現在は「テーブルを開く時間のほぼ全て」になっている。列ごとの逐次実行を並列化すればコア数近くまでスケールする可能性がある(上記「summariseの列並列化」の項目参照、未検証)。次の対応候補として優先度を引き上げる。
