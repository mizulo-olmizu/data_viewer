...
- **① サーバー側ページング化(最優先、2026-07-27に優先度を引き上げ)**: JSON→Arrowバイナリ転送(②)を検証した結果、エンコード方式の工夫だけでは頭打ちになることが判明したため、既存UX（即時クライアントサイドソート/フィルタ）を変える設計判断を伴う本命の対応として優先度を最上位に引き上げた。詳細は下記「サーバー側ページング化の再検討」を参照。仮想化ライブラリの統一(issue #8、`react-virtuoso`→`@tanstack/react-virtual`)は既に対応済みのため、これと合わせた設計は不要になった。
