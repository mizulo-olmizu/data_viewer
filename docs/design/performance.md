...
### ✅ メインテーブル表示: 全件ロード + JSON多段変換(いずれも対応済み)
...
- ✅ **サーバー側ページング化(2026-07-27対応済み)**: `Table.tsx`/`GlimpseView.tsx`/`RecordView.tsx`が`fetch_table_page`でoffset/limit/sort/whereごとに分割取得する設計に移行し、`SELECT * FROM {table_name}`で常に全件取得していた旧`extract_table`は廃止した。詳細は下記「サーバー側ページング化 本格実装」を参照。

### ✅ numeric_summariseでのヒストグラム用生データ全送信(2026-07-28対応済み)

- `numeric_summarise`は統計量・SQL集計済みのビンに加え、`extract_raw_column`で列の生データも丸ごとRust側に取得しフロントへ送信していた。`HistogramChartInteractive`のインタラクティブ再ビニング(ビン数変更・範囲フィルタ)に使われていたため単純に送信をやめると機能が壊れる状態だったが、この再ビニング自体をバックエンドへの再クエリ(新設`get_numeric_bins`コマンド)に切り替えることで、生データ送信(`raw`/`numericRaw`/`extract_raw_column`)を完全に廃止した。value_countsの上限化(下記)も同じ対応の中で実施した。詳細は下記「ヒストグラム/棒グラフのインタラクティブ機能をサーバー側計算に移行」を参照。

### ⬜→見送り カラム集計・クエリ実行のたびの複数回フルスキャン

- `numeric_summarise`はカラムごとにstats/binningで2回(生データ送信廃止によりraw取得分は無くなった)、`string_summarise`は最大3回(stats+value_counts_limitedの2クエリ)、独立したSQLを発行している。カラム数×複数回のスキャンになる。
- **(2026-07-28追記、見送りとしてクローズ)** 統合を検討したが、以下の理由により見送ることにした:
  - stats(単純な集計)・binning(ウィンドウ関数を使うヒストグラムCTE)・value_counts(GROUP BY+LIMIT+OFFSET集計)は形の異なるクエリであり、1つに統合しようとすると可読性・保守性を大きく犠牲にする。
  - `summarise`の列並列化(コア数までスケール)とサーバー側ページング化により、`build_table_metadata_json`全体は既に`flights.csv`(33万行・19列)で114〜305ms程度まで縮んでいる。個々のクエリ自体も数ms〜数十ms(本ドキュメント冒頭の実測参照)であり、クエリ本数を減らして削れる余地は小さいと判断。
  - 将来、実測で再びボトルネックだと判明した場合に限り再検討する(現時点では対応しない)。
...
- ✅ **② 生データ送信の削減/ヒストグラムのサーバー側計算化(2026-07-28対応済み)**: 「DuckDB側で計算し、フロントには表示分だけ渡す」という①と同じ方針をチャート/集計全般に広げた。インタラクティブ再ビニングは生データ送信をやめ、ビン数変更のたびにバックエンドへ再クエリする設計に変更した(あわせて`value_counts`の上限化も実施)。詳細は下記「ヒストグラム/棒グラフのインタラクティブ機能をサーバー側計算に移行」を参照。
- ⬜→見送り **複数回スキャンの削減**(カラムごとにstats/binning/value_countsを個別クエリで発行している点)は、①②③対応済みにより`build_table_metadata_json`全体が既に114〜305ms程度まで縮んでいること、クエリの形が異なり統合が可読性・保守性を大きく犠牲にすることから、見送りとしてクローズした(2026-07-28)。詳細は上記「現状の課題」節を参照。
