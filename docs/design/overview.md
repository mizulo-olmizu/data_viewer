...
- メインテーブル表示のページング化・変換パイプラインの見直し(サーバー側ソート/フィルタへの切り替えを伴う設計変更、最優先)。仮想化ライブラリの統一(issue #8、`react-virtuoso`→`@tanstack/react-virtual`への一本化、Grid表示の作り直しを伴う)もこの作業と合わせて設計する(パディング化に伴いどのみちTable.tsxの仮想化まわりに手を入れる必要があるため、一度にまとめる方が手戻りが少ない)。
...
- DuckDB接続へのメモリ上限・スレッド数設定の追加。既存UXに影響しない低リスクな改善のため、上記のページング化と独立して先に着手できる。
