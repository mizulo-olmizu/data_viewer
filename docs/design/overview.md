...
🟡 行数・列数も表示される。（`App.tsx`にバッジ表示はあるが、列数バッジのラベルが誤って「Rows」になっているバグあり）
...
- ⬜ SQLへのテーブル名やカラム名のコピーができるようにする。
  - サイドバーはテキスト表示のみで、クリック/コピー操作やSQLエディタとの連携は無い。
...
- 🟡 画面右側のサイドパネル。出し入れ、ドラッグでサイズ調整ができる。
  - 現状は右からスライドする`Sheet`（ドロワー）実装で、開閉はできるがドラッグでのサイズ調整は無い。
...
| テーブル表示(表形式) | Table描画、仮想スクロール | 行/列数表示(バグ)、サイドバーリサイズ、ソート(フィルタ/カラム選択欠) | クリップボード/CSV、スキーマ名コピー |
| glimpse表示 | - | - | 全体 |
| 1行表示 | - | - | 全体 |
...
| SQL Editor | 実行/補完/フォーマット/デフォルトクエリ/`_last`保存 | パネルのドラッグリサイズ | クエリ履歴 |
