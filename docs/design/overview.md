...
2次元要約が加わり集計クエリが増えたタイミングで、既存分と合わせて速度面の課題をまとめて洗い出し・対応する。詳細・現状把握は `performance.md` を参照。
- メインテーブル表示のページング化(サーバー側ソート/フィルタへの切り替えを伴う設計変更)。
- ヒストグラム用の生データ全送信の削減(ビンが既にSQL側で計算済みのため不要)。
- カラム集計・`execute_query`実行時の複数回フルスキャンの削減(集計クエリの合成)。
- DuckDB接続へのメモリ上限・スレッド数設定の追加。
