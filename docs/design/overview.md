...
- **summariseの列並列化(優先度を引き上げ、2026-07-28)**: ページング化により`get_table_metadata`がdf(全行データ)を含まなくなった結果、summarise(19列合計で約1.6秒)がテーブルを開く際の支配的コストになった。列ごとの逐次実行(`schema.iter().map(...)`)を並列化する対応の優先度を引き上げる(未着手)。
