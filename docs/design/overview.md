...
| パフォーマンス(メモリ効率・速度) | 1次元要約のSQL集計プッシュダウン、Arrow→JSON変換パイプラインの高速化・IPC二重エスケープ解消 | - | メインテーブルのページング化(最優先、JSON→Arrowバイナリ転送は検証の上で見送り)、ヒストグラム生データ送信の削減、集計クエリの複数回スキャン削減。詳細は`performance.md` |
...
- **メインテーブル表示のサーバー側ページング化(最優先、2026-07-27に優先度を引き上げ)**。上記の検証により、エンコード方式の工夫だけでは大規模データの速度課題を解決できないことが分かったため、サーバー側ソート/フィルタへの切り替えを伴う設計変更に本格的に着手する方針とした。DBeaver/TablePlus等の大規模データ対応ツールやPerspective(FINOS)等が採る「エンジン側で処理し、フロントには表示分だけ渡す」設計に倣う。影響範囲(`Table.tsx`のデータモデル、グローバル検索/高度フィルタ、セル範囲選択・コピー、CSVエクスポート、Glimpse/RecordView)の洗い出しから着手する。詳細は`performance.md`の「サーバー側ページング化の再検討」を参照。
