...
- 🔍 データが大きすぎる場合、一部のみを取り込む(LIMIT追加する)ように促すダイアログを検討したが、対応不要と判断(2026-07-28)。サーバー側ページング化・summariseの列並列化・ヒストグラム/棒グラフのサーバー側計算化により大規模データでも十分な速度が出るようになり、データ取り込み時に確認を挟む必要性が薄れたため。関連して、実装されないまま設定画面に残っていた未使用の`limitDialogThreshold`設定項目も削除した。
...
| データのインプット | アップロード/D&D/Finder/SQL/CLI/HTTP(登録) | - | - |
...
| 設定 | 設定画面新設、テーマ手動切り替え、infer_schema_lengthデフォルト、起動時最前面化on/off、コピー時行番号・列名付与on/off、HTTPサーバーのポート番号(即時反映) | プライマリ/セカンダリカラー(項目のみ) | その他の定数化箇所 |
...
- ✅ 設定画面の新設（テーマ手動切り替え、プライマリ/セカンダリカラー、infer_schema_lengthのデフォルト値、起動時最前面化のon/off、Tableのセル範囲コピー時に行番号・列名を付与するかのon/off）。プライマリ/セカンダリカラーは項目のみで実装は次タスク。詳細は下記「実装メモ」参照。
...
- 🔍 LIMIT確認ダイアログの実装（大規模データ取り込み時）を検討したが、対応不要と判断(2026-07-28)。サーバー側ページング化・summariseの列並列化・ヒストグラム/棒グラフのサーバー側計算化により大規模データでも十分な速度が出るようになり、必要性が薄れたため。設定画面にあった未使用の閾値項目(`limitDialogThreshold`)も削除した。詳細は上記「実現したい機能」の「データのインプット」参照。
...
- 🔍 カラム集計・`execute_query`実行時の複数回フルスキャンの削減(集計クエリの合成)を検証したが見送り(2026-07-28)。stats/binning/value_countsは形の異なるクエリであり統合は可読性・保守性を大きく犠牲にする一方、①②③対応済みにより`build_table_metadata_json`全体は既に114〜305ms程度まで縮んでおり、得られる利益は小さいと判断した。詳細は`performance.md`参照。
