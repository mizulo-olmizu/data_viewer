...
- **ヒストグラム用生データ全送信の削減/チャートのサーバー側計算化(方針決定、2026-07-28)**: 生データは`HistogramChartInteractive`のインタラクティブ再ビニングに使われているため単純に削除すると機能が壊れる。ユーザーと相談の結果、「DuckDB側で計算し、フロントには表示分だけ渡す」という①(ページング化)と同じ方針をチャート/集計全般に広げる方向で合意した。既存のヒストグラム再ビニングは生データ送信をやめ、ビン数変更のたびにバックエンドの`binning`へ再クエリする設計に変更する。**未実装の2次元要約(フェーズ3)も、この方針(生データではなく集計結果のみ渡す)を前提に設計する**。次のセッションで着手する(未着手)。
- カラム集計・`execute_query`実行時の複数回フルスキャンの削減(集計クエリの合成)。上記のヒストグラム生データ送信削減に着手すればraw取得分のクエリが1本減るため、合わせて設計するのが手戻りが少ない。単独での優先度は低いまま。
