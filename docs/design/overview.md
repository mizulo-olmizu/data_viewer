...
| パフォーマンス(メモリ効率・速度) | 1次元要約のSQL集計プッシュダウン、Arrow→JSON変換パイプラインの高速化・IPC二重エスケープ解消、メインテーブル表示(Grid/Glimpse/Record)のサーバー側ページング化 | - | ヒストグラム生データ送信の削減、集計クエリの複数回スキャン削減。詳細は`performance.md` |
