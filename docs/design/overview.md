...
内容が膨らんできたら、フロントエンド固有の詳細は `frontend.md`、バックエンド固有の詳細は `backend.md` に切り出す。メモリ効率・速度に関する課題は `performance.md` にまとめている。
...
- ✅ データベースはUI上から永続化できる。
  ...
  - Tauri IPC(UI)のみ対応。HTTP/CLIからの保存は、他インターフェースとの機能パリティ整備(フェーズ5)の一環として改めて検討する。
...
- ⬜ 列幅のドラッグリサイズ。現状は固定/自動幅のみで、個別カラムの幅を手動調整する手段が無い。フェーズ2でサイドパネル(スキーマ・SQLエディタ)のドラッグリサイズと合わせて実装予定(下記「今後の計画」参照)。
- ✅ クリップボードに保存、CSVなどでのダウンロード。
  ...
  - ✅ コピー時の行番号・列名付与をon/offする設定を追加(設定画面、`localStorage`に永続化)。`useCellRangeSelection`に`includeHeaders`オプションとして渡し、Grid(Table.tsx)・Glimpse(GlimpseView.tsx)双方に反映される。
...
- 🟡 個々の機能(補完・フォーマット・lint等)は実装済みだが、エディタ全体としての完成度・使い勝手にはまだ詰め切れていない部分がある、という課題認識がある。フェーズ2で何が足りないか改めて棚卸しし、クエリ履歴・パネルのドラッグリサイズと合わせて拡充する予定(下記「今後の計画」参照)。
...
### 設定
- 🟡 各種設定ができる。
  - ✅ 設定画面(`SettingsDialog`)を新設(`feature/settings-screen`ブランチ)。サイドバーヘッダーの歯車アイコンから開く。詳細は下記「実装メモ」の「設定画面の新設」を参照。
  - 🟡 デザイン(プライマリ・セカンダリカラー、ライト・ダーク)
    - ライト/ダーク/システムの手動切り替えUIを設定画面に追加。プライマリ・セカンダリカラーは設定画面上にプレースホルダー項目のみ配置("Coming soon")、実装は次の「配色更新」タスクで行う。
  - ✅ LIMITのダイアログが出る件数
    - 閾値の入力欄を設定画面に追加。ダイアログ本体(閾値を超えた際の確認)はまだ無く、別タスクで実装予定。
  - ✅ infer_schema_length(型を推論するために読み込む行数)
    - 設定画面でDefault/Inf/Customを選択できる。UIからのアップロード/ドラッグ&ドロップ(`registerData`呼び出し)にはこの値が反映される。CLI/HTTP経由のデフォルト値には影響しない(既存通りリクエストごとのパラメータのみ)。
  - ✅ 起動時に、何か外部から操作があったら最前面に来るようにするか。
    - 設定画面のトグルで切り替え可能。single-instance再起動/HTTPハンドラの`window.set_focus()`呼び出しを実際に制御する。
  - 設定はすべてRust側(Tauriのapp config dir配下`settings.json`)1ファイルに永続化(`get_settings`/`set_settings`コマンド経由)。詳細は下記「実装メモ」参照。
  - ✅ HTTPサーバーのポート番号
    - 「次回起動時のデフォルト」と「今まさに稼働中のポート」を別物として扱う。設定画面で変更できるのは前者(`settings.json`に永続化、次回起動時のみ反映)。優先順位は起動時CLI引数`-p/--port`(その起動限りの一時上書き、設定への書き戻しはしない) > 設定画面の値 > デフォルト値(3000)。
    - 稼働中のポートを直接切り替えたい場合は、サイドバーのURL表示横の鉛筆アイコンから入力するUIで行う。入力途中の値では切り替わらず、Enter/ボタン押下で確定した時点でのみ切り替わる(専用の`switch_http_port`コマンド)。HTTPサーバーが停止中(下記参照)でも同じUIからポートを指定して起動できる。このコマンドは実際のbind結果を待ってから返るため、失敗時はグローバルなエラーダイアログではなく入力欄直下にインラインでエラーが表示され、ポップオーバーも閉じない。
    - 既に起動中のアプリに対する`-p`付きCLI再実行(single-instance経路)は、確認なしで即座にaxumサーバーを新ポートで再起動する(`tokio::sync::mpsc`チャネルで駆動、こちらは結果を待たないfire-and-forget)。
    - 上記いずれの切り替えも、成功時はサイドバーのURL表示に加えてトースト通知でも明示する。
    - 新ポートへの`bind`に先に成功してから旧サーバーを止める設計になっているため、指定したポートが既に他プロセスに使われていても、今まで通りのポートでサーバーは動き続ける。起動時の初回bindが失敗した場合も同様にリトライ待機状態に入り、別ポートを指定し直せば復帰できる。ただし、bind以外の理由でHTTPサーバーが予期せず落ちた場合(頻度は低い)は自動リトライの対象外で、復帰にはアプリの再起動が必要。
  - ⬜ その他、定数化しているところなど
  - セル範囲コピー時に行番号・列名を付与するかのon/offも設定画面に追加(`useCellRangeSelection`の`includeHeaders`オプションとして反映、詳細は「テーブル表示」節参照)。
...
- 大規模なデータが入力される可能性があるため、データの取り込みや、表示、表示切り替えの際などに、なるべく待ち時間が発生しないようにする。処理時間がなるべく短くなるようにする。
  - 現状のメモリ効率・速度面の課題の洗い出しと対応方針は `performance.md` を参照。
...
| テーブル表示(表形式) | Table描画、仮想スクロール、行/列数表示、スキーマ名コピー/挿入、フィルタ(全体検索+Excelライクな高度なフィルタ)、カラム表示切り替え、ソート(3状態リセット込み)、列の並び替え(D&D)/Pin/一括リセット、常時表示の行番号列、セル範囲選択コピー(TSV)/CSVダウンロード | サイドバーリサイズ、Columnsボタンの表示遅延(Issue #3) | 表示フォーマットのカスタマイズ、列幅のドラッグリサイズ |
...
| 要約表示(1次元) | 数値/文字列/日付/Bool の chart+統計量、ツールチップ、カラム表示切り替え、ヒストグラムのビン数変更・範囲フィルタ/カテゴリ列のOn-off切り替え(いずれもサーバー側計算、2026-07-28対応済み) | - | 2次元要約全パターン、AI向けテキストコピー |
| データのインプット | アップロード/D&D/Finder/SQL/CLI/HTTP(登録) | - | LIMIT確認ダイアログ |
| SQL Editor | 実行/補完/フォーマット/デフォルトクエリ/`_last`保存/非モーダル押しのけパネル | パネルのドラッグリサイズ、エディタ全体としての完成度(要棚卸し) | クエリ履歴 |
| インターフェース | シングルトン化、外部操作での起動/最前面化 | HTTP・CLI(登録のみ、SQL実行不可)、Python lib | MCP、R lib(実装コード無し) |
| パフォーマンス(メモリ効率・速度) | 1次元要約のSQL集計プッシュダウン、Arrow→JSON変換パイプラインの高速化・IPC二重エスケープ解消、メインテーブル表示(Grid/Glimpse/Record)のサーバー側ページング化、summariseの列並列化、ヒストグラム生データ送信の削減とvalue_countsの上限化 | - | 集計クエリの複数回スキャン削減。詳細は`performance.md` |
| 設定 | 設定画面新設、テーマ手動切り替え、infer_schema_lengthデフォルト、LIMIT閾値、起動時最前面化on/off、コピー時行番号・列名付与on/off、HTTPサーバーのポート番号(即時反映) | プライマリ/セカンダリカラー(項目のみ) | その他の定数化箇所 |
...
- ✅ 設定画面の新設（テーマ手動切り替え、プライマリ/セカンダリカラー、LIMITダイアログ閾値、infer_schema_lengthのデフォルト値、起動時最前面化のon/off、Tableのセル範囲コピー時に行番号・列名を付与するかのon/off）。プライマリ/セカンダリカラーは項目のみで実装は次タスク。詳細は下記「実装メモ」参照。
...
- 配色・コンポーネントの質感をデザイン方針に沿って更新（ニュートラルカラーの見直し、アクセントカラーの適用範囲決定）。
  - あわせて、設定画面の画面構成も再検討する。現状はフロートするモーダル(`Dialog`)で、項目数が増えるとスクロールも効かず窮屈になる。他の機能(コマンドパレット案なども含む)とのUI全体のバランスを見ながら、独立した画面にするかどうかをこのタイミングで判断する。
...
- SQLエディタの機能拡充。クエリ履歴だけでなく、DuckDB SQLの補完・フォーマット・lintを含め、エディタとしての完成度が低い(細部が詰め切れていない)という課題認識があるため、まずは何が足りないか・何を実装するかの棚卸し・検討から着手する。
- パネルのドラッグリサイズ（スキーマパネル・SQLエディタパネル）と、Tableの列幅リサイズ(ドラッグでの個別カラム幅変更、現状は固定/自動幅のみ)をあわせて実装する。どちらもドラッグによるリサイズ操作という点で共通するため、まとめて着手する。
...
- 🔍 JSON→Arrowバイナリ転送への変更を検証したが見送り(`feature/perf-timing-logs`ブランチ)。Rust側の変換は約4倍(`flights.csv`33万行、205〜234ms→49〜54ms)速くなったが、フロント側(JS)のデコードコストが逆に旧JSON方式の4倍以上(約330ms→約1,335ms)に悪化し、トータルでは明確な劣化だったため実装ごと差し戻した。詳細・原因分析は`performance.md`の「② JSON→Arrowバイナリ転送への変更 検証したが見送り」を参照。この検証を通じて「エンコード方式の工夫では頭打ちになる、サーバー側ページング化が本命」という結論に至った。
- ✅ サーバー側ページング化のPOC(技術検証、`feature/perf-timing-logs`ブランチ)。本格実装(セル範囲選択・CSVエクスポート・RecordView等の大幅な再設計を伴う)に入る前に、`Table.tsx`本体とは隔離した「Paged (POC)」タブで、DuckDB側のOFFSET/LIMIT/ORDER BY/WHERE処理とスクロール時の追加フェッチの技術的実現性のみを検証した。`flights.csv`(33万行)で深いオフセットへのスクロール・ソート併用・WHERE句・通常スクロールいずれも問題なく、実現性が確認できた。詳細は`performance.md`の「サーバー側ページング化 POC(技術検証)の結果」を参照。
- ✅ **メインテーブル表示のサーバー側ページング化・本格実装(2026-07-27対応済み、`feature/perf-timing-logs`ブランチ)**。DuckDB側でOFFSET/LIMIT/ORDER BY/WHEREを処理し、フロントには表示中の少量データだけ渡す設計に移行した。Grid(`Table.tsx`)・GlimpseView・RecordViewの3ビューすべてが対象(GlimpseViewも含めるようユーザーから明示的な指示があった)。「プレースホルダー配列」設計(`src/usePagedRows.ts`)によりTanStack Tableの列状態管理(D&D/Pin/表示切り替え)を無改造で流用し、既存機能への回帰を避けた。セル範囲選択のコピーは1クエリでの直接取得+大量選択時の確認ダイアログ、CSV全件エクスポートはDuckDBの`COPY TO`直接実行に置き換えた。詳細は`performance.md`の「サーバー側ページング化 本格実装」、データフローの詳細は`CLAUDE.md`の「フロントエンドのデータフロー」を参照。
  - 検証中、GlimpseView/RecordViewで深い未訪問位置へ一気にジャンプする操作でWebKitがクラッシュしウィンドウが再初期化される現象を発見したが、原因はReact 19開発ビルド固有の`performance.measure()`計測機能とこのマシンのWebKitバージョンの組み合わせによる環境要因(本番ビルドでは未再現)と判明した。詳細は`performance.md`参照。
- ✅ 仮想化ライブラリの統一(issue #8、`react-virtuoso`→`@tanstack/react-virtual`への一本化、`feature/perf-timing-logs`ブランチ)。`TableVirtuoso`を`useVirtualizer`+spacer`<tr>`方式に置き換え、既存のPin列・D&D列並び替え・セル範囲選択・CSVエクスポート等は無回帰で移行できた。速度改善そのものが目的ではなく、ライブラリ統一・計測の土台安定化が主目的(前後のレンダリング時間を計測はしたが、1回ずつの計測でしかなく、このマシンでの計測値は系の負荷次第で大きくばらつくことが分かっているため、速度改善の根拠としては扱わない)。詳細は`performance.md`参照。
...
- **(訂正、2026-07-28追記)** 以下2件は「サーバー側ページング化がまだ支配的コストだった前提」での優先度付けだったが、ページング化対応済みにより前提が変わった。詳細は`performance.md`の「対応方針・優先順位」節の訂正を参照。
- ✅ **summariseの列並列化(2026-07-28対応済み、`feature/perf-timing-logs`ブランチ)**: ページング化により`get_table_metadata`がdf(全行データ)を含まなくなった結果、summarise(19列合計で約1.6秒)がテーブルを開く際の支配的コストになっていた。`DbState::summarise_all`を新設し、`Connection::try_clone()`(同一データベースを共有する別コネクション)を`rayon::current_num_threads()`で頭打ちにした数だけ用意し、列をワーカー数でチャンク分割して並列実行するよう変更した(列数分そのままコネクションを複製すると列幅の広いテーブルで無駄が大きいため)。`flights.csv`(33万行・19列)での実測で、`build_table_metadata_json`全体が約1.7〜2.3秒→114〜158msに短縮した(このマシンの計測値は系の負荷次第でばらつくため、目安として扱う)。詳細は`performance.md`の「summariseの列並列化 実施後の再計測結果」を参照。
- ✅ **ヒストグラム/棒グラフのインタラクティブ機能をサーバー側計算に移行(2026-07-28対応済み、`feature/perf-timing-logs`ブランチ)**: `numeric_summarise`/`temporal_summarise`から生データ送信(`raw`/`numericRaw`)を完全に廃止し、ヒストグラムモーダルのビン数変更・範囲フィルタは新規コマンド`get_numeric_bins`でパラメータ変更のたびにバックエンド(DuckDB)へ再クエリする設計に変更した(`src/useNumericBins.ts`、`usePagedRows.ts`と同じデバウンス+generationカウンタ規約を踏襲)。あわせて、`value_counts()`にLIMITが無く高カーディナリティな文字列列で実質行数分の値リストが送られうる問題も発見し、同じ対応の中で解消した: `string_summarise`は新設の`DbState::value_counts_limited`(上位50件+Other1件、2クエリ構成でOtherより後の行をRust側に読み込まない設計)を使うよう変更。`ValueCount<T>`に`isOther`フラグを追加し、`ValueCountsChart`の位置ベースだった`otherIndex`判定(バグ: `otherIndex===0`でfalsyになる)も併せて根本修正した。「生データではなく集計結果のみを渡し、パラメータ変更時はバックエンドへ再クエリする」という統一パターンを確立し、未実装の2次元要約(フェーズ3)や今後の対話的チャートもこれに倣う方針。詳細は`performance.md`の「ヒストグラム/棒グラフのインタラクティブ機能をサーバー側計算に移行」を参照。
- カラム集計・`execute_query`実行時の複数回フルスキャンの削減(集計クエリの合成)。単独での優先度は低いまま。
...
- 前提・課題: フェーズ2最初のタスク。`DbState::save_database`はRust側に実装済みだったがコマンド化されておらずUIから呼べなかった。また起動時に永続DBを読み込む手段(CLI引数)も無かった。ユーザーとの相談の結果、対応スコープをTauri IPC(UI)のみに絞り(HTTP/CLIでの保存はフェーズ5の機能パリティ整備時に改めて検討)、加えて「開く」も対称的に実装する方針にした。
- 対応方針・実施内容:
...
### 設定画面の新設 ✅ 対応済み(2026-07-26)
- 前提・課題: フェーズ2最初のタスク。設定画面・設定ストア自体が存在せず、永続化されているクライアント側状態はテーマ文字列(`localStorage`)のみだった。プライマリ/セカンダリカラーのカスタマイズは、実装スコープをどこまで含めるかユーザーと相談した結果、設定画面には項目のみ用意し実装は次の「配色更新」タスクに回すことにした。
- 対応方針・実施内容:
  - 設定は最終的にRust側1箇所(Tauriのapp config dir配下`settings.json`)に統一して永続化している。当初はフロントエンド完結の設定を`localStorage`、`focusOnExternalUpdate`だけをRust側、という2層構成で実装したが、レビューで「保存場所が分かれていて分かりにくい」という指摘を受け、テーマも含めて全設定をRust側の`settings.json`1ファイルに統一する形に作り直した。
    - `AppData`(`src-tauri/src/modules/handler.rs`)は設定の中身を`serde_json::Value`として不透明に保持する(`pub settings: serde_json::Value`)。Rust側が構造を意識するのは`focusOnExternalUpdate`フィールドだけで、それ以外のキーはフロントエンドが決めた形をそのまま素通しする。汎用の`get_settings`/`set_settings`コマンドの2つだけで全設定の読み書きが完結する(設定項目が増えてもRust側のコマンド追加は不要)。
    - `focusOnExternalUpdate`だけRust側で解釈が必要なのは、single-instance再起動ハンドラ・HTTPの`/update-data`ハンドラの2箇所にある`window.set_focus()`呼び出しを制御する必要があり、フロントエンドの状態だけでは(single-instance再起動はフロントのJS実行を経由しないため)届かないため。`AppData::focus_on_external_update()`というヘルパーメソッドで`settings`から都度取り出す形にし、専用フィールドとして二重管理はしていない。
    - フロントエンドは`SettingsProvider`(`src/components/settings-provider.tsx`)が起動時に`get_settings`を呼んで初期値を読み込み、変更のたびに`set_settings`で書き戻す。既存の`ThemeProvider`/`useTheme`(`src/components/theme-provider.tsx`/`src/hooks/use-theme.ts`)は廃止し、テーマの状態と副作用(`<html>`へのdark/lightクラス付与)も`SettingsProvider`/`useSettings`(`src/hooks/use-settings.ts`)に統合した。
    - 初期ロードが非同期(Tauri IPC)になったため、`localStorage`のような同期読み込みができない。設定が読み込まれるまで`SettingsProvider`は子要素を描画しない(`null`を返す)ことで、誤ったテーマでの一瞬の描画(ちらつき)を避けている。
  - infer_schema_lengthのデフォルト値は、UIからのアップロード/ドラッグ&ドロップ(`App.tsx`の`handleOnFileChange`、唯一の登録経路)にのみ反映する。設定値(`Default`/`Inf`/`Custom`)を`registerData`の`options`マップ(`sample_size`キー、DuckDBの`read_csv`等が受け取る既存の仕組みにそのまま乗る)に変換するだけの純粋関数`inferSchemaLengthToOptions`を`src/utils.ts`に追加した。CLI/HTTP経由のデフォルト値には影響しない(既存通りリクエストごとのパラメータのみ)。
  - セル範囲コピーの行番号・列名付与on/offは、`useCellRangeSelection`(`src/useCellRangeSelection.ts`)に`includeHeaders`オプションを追加し、Grid(`Table.tsx`)・Glimpse(`GlimpseView.tsx`)の両呼び出し元から`useSettings()`の値をそのまま渡すだけにした。
  - 設定画面(`SettingsDialog.tsx`)は既存の`components/ui/dialog.tsx`を使ったモーダルとし、タブは使わずセクション区切り(表示/データ取り込み/テーブル表示/アプリケーション)の1画面リストにした(デザイン方針の「情報密度」を優先)。入口はサイドバーヘッダーの「Database」ドロップダウンの隣に置いた歯車アイコンボタン(`app-sidebar.tsx`)。
  - `App.tsx`は`SettingsProvider`(および`useSettings()`)を使うために、旧来の`App`関数の中身を`AppContent`に切り出し、新しい`App`は`SettingsProvider`の設置と`<AppContent />`のレンダリングのみを担う薄いラッパーにした。`AppContent`自身は自分の親である`SettingsProvider`のcontextを読めない(contextはレンダーツリー上の子孫からしか参照できない)ため、この分割が必要だった。
- ハマりどころ:
  - Rust側で`focusOnExternalUpdate`をチェックする2箇所(single-instance再起動、HTTPハンドラ)は、いずれも`window.set_focus()`を呼ぶ時点で既に`Mutex<AppData>>`のロックが外れている(`register_data`呼び出しのブロックスコープを抜けた後)ため、フォーカス直前で改めて`state.lock()`し直す必要があった。使い回そうとすると借用エラーになる。
  - infer_schema_lengthのデフォルト値を検証する過程で、DuckDBのCSV auto-detectは`sample_size`をそのまま使わず、内部の実行チャンクサイズ(`STANDARD_VECTOR_SIZE`=2048行)の倍数に切り上げる(非公式・ドキュメント未記載の挙動、実測で確認)ことが分かった。Customで小さい値を指定しても実際にはそれ以上スキャンされるが、Customの用途が「defaultの20,480行で拾いきれなかった場合に増やしたい」というケースであるため、切り上げられる分には実害が無いと判断し、UI側の特別な対応(注記や刻み幅の変更など)はしないことにした。
- 既知の未対応事項(次の「配色・コンポーネントの質感を更新」タスクで併せて検討予定): 設定項目が今後増えた場合、現状のフロートするモーダル(`Dialog`)ではスクロールが効かず窮屈になる。現時点ではその対応はせず、画面構成そのものを見直すタイミング(フェーズ2の配色更新タスク)まで先送りする。
...
- 効果: 速度改善そのものが目的の対応ではない(ライブラリ統一・計測の土台安定化が主目的)。`flights.csv`(33万行)でのフロントレンダリング時間(state更新→再描画完了)を移行前後で1回ずつ計測したところ約2,987ms→約1,466msだったが、このマシンでの計測値は系の負荷次第で大きくばらつくことが分かっており(`performance.md`参照)、この数値差をライブラリ変更の効果として主張できる根拠は無い。
